...
public class Profile
{
    ...
    public Profile(ArrayList knownSegments, ArrayList knownGases, AbstractModel m)
    {
        ...
        if (m==null) {  
            ...
            // Define which concrete model to use
             try {                
                if (Mvplan.DEBUG>0) System.out.println("Loading model class:"+Mvplan.prefs.getModelClass());
                modelClass = Class.forName(Mvplan.prefs.getModelClass());
                model = (AbstractModel)modelClass.newInstance();
            } catch (Exception ex) {
                model = new ZHL16B();
                if (Mvplan.DEBUG>0) System.out.println("Model instantiation exception for: "+ex.getMessage());
            }
        ...
        while(ig.hasNext()) {
            ...
            if (g.getEnable()==true) {
                ...
                if(Mvplan.DEBUG >1) System.out.println("Adding gas "+g);
                gases.add(g);            
            }
        }
    }
    ...
    public int doDive()
    {
        ...
        double t;
        double deltaDepth;
        boolean runtimeFlag=Mvplan.prefs.getRuntimeFlag();      // Used to decide if segment represents runtime or segtime
        ...
        while(it.hasNext()) {
            ...
            if (Mvplan.DEBUG >1) System.out.println("Processing: "+s);
            if(s.getType() == SegmentAbstract.CONST) {      // Should be constant depth segments only
                ...
                if(deltaDepth>0.0) {  // Segment causes a descent
                    try {
                        model.ascDec(currentDepth,sd.getDepth(),Mvplan.prefs.getDescentRate(),currentGas.getFHe(),currentGas.getFN2(),ppO2);
                    } catch (ModelStateException e) { 
                        return PROCESSING_ERROR;  
                    }
                    // Add segment to output segments
                    outputSegments.add(new SegmentAscDec(currentDepth,sd.getDepth(),Mvplan.prefs.getDescentRate(),currentGas,ppO2));
                    runTime+=deltaDepth/Mvplan.prefs.getDescentRate();
    ...
    public int ascend(double target)
    {
        ...
        if (Mvplan.DEBUG > 1) System.out.println("\nASCEND: started ascent to: "+target);
        if (Mvplan.DEBUG > 1) System.out.println("RT: "+runTime+" ppO2: "+ppO2);        
        /*
        ...
        if(inFinalAscent && (Mvplan.prefs.getOcDeco()))  {      // Switch to Open circuit deco  
            currentGasIndex=-1;
        ...
        if ((currentDepth%(int)Mvplan.prefs.getStopDepthIncrement()) > 0)   // Are we on a stop depth already ?
            // If not, go to next stop depth
            nextStopDepth= (int)(currentDepth /  Mvplan.prefs.getStopDepthIncrement()) * (int)Mvplan.prefs.getStopDepthIncrement();
        else
            nextStopDepth = (int)(currentDepth - Mvplan.prefs.getStopDepthIncrement());           
           
        // Check in case we are overshooting or hit last stop or any of the other bizzar combinations ...
        if((nextStopDepth < target) || (currentDepth < Mvplan.prefs.getLastStopDepth()))
            nextStopDepth=target;
        else if(currentDepth==Mvplan.prefs.getLastStopDepth()) 
            nextStopDepth=target;
        else if(nextStopDepth < Mvplan.prefs.getLastStopDepth()) 
            nextStopDepth=Mvplan.prefs.getLastStopDepth();
        ...
        if (Mvplan.DEBUG > 1) System.out.println("Initial stop depth: "+nextStopDepth);        
        if (Mvplan.DEBUG > 1) System.out.println(" ... ceiling is now:"+model.ceiling());
        if (Mvplan.DEBUG > 1) System.out.println(" ... set m-value gradient for: "+nextStopDepth);
        ...
        while (currentDepth > target) {              
            ...
            while (forceDecoStop || (nextStopDepth < model.ceiling())) {
                ...
                if (Mvplan.DEBUG > 1) System.out.println(" ... entering decompression loop ...");
                inDecoCycle=true;
                ...
                if(inAscentCycle) {     // Finalise last ascent cycle as we are now decompressing
                    if(startDepth > currentDepth)   // Did we ascend at all ?
                        // Add ascent segment 
                        outputSegments.add(new SegmentAscDec(startDepth,currentDepth,Mvplan.prefs.getAscentRate(),currentGas,ppO2));  
                    inAscentCycle=false;
                ...
                if ((!Mvplan.prefs.getGfMultilevelMode() || inFinalAscent) && !model.getGradient().isGfSet()) { 
                    model.getGradient().setGfSlopeAtDepth(currentDepth);
                    if (Mvplan.DEBUG > 1) System.out.println(" ... m-Value gradient slope set at: "+currentDepth+" GF is:"+ model.getGradient().getGf());                      
                    model.getGradient().setGfAtDepth(nextStopDepth);
                    if (Mvplan.DEBUG > 1) System.out.println(" ... set m-value gradient for: "+nextStopDepth+" to:"+model.getGradient().getGf());                                        
                }
                
                // Round up runtime to integral number of minutes - only first time through on this cycle
                if( decoStopTime==0 && (runTime%Mvplan.prefs.getStopTimeIncrement() > 0))  // Is this not an integral time
                    stopTime=(int)(runTime/Mvplan.prefs.getStopTimeIncrement())*Mvplan.prefs.getStopTimeIncrement() +
                            Mvplan.prefs.getStopTimeIncrement()-runTime;
                else 
                    stopTime=Mvplan.prefs.getStopTimeIncrement();
                
                // Sanity check the rounding
                if(stopTime==0) stopTime=Mvplan.prefs.getStopTimeIncrement();
                
                if (Mvplan.DEBUG > 1) System.out.println(" ... decompressing at depth: "+
                ...
                if(decoStopTime > 5000) {
                    if (Mvplan.DEBUG > 0) System.err.println("Infinite loop on deco stop at "+currentDepth);
            ...
            if(inDecoCycle) {
                ...
                if(Mvplan.prefs.getForceAllStops())     // Reset for next depth
                ...
                if (Mvplan.DEBUG > 1) System.out.println(decoSegment);
                inDecoCycle=false;    
                decoStopTime=0;            
                ...
                try {
                    model.ascDec(currentDepth,(double)nextStopDepth,Mvplan.prefs.getAscentRate(),currentGas.getFHe(),currentGas.getFN2(),ppO2);
                } catch (ModelStateException e) { return PROCESSING_ERROR; }
                runTime+=(currentDepth-nextStopDepth)/(-1.0*Mvplan.prefs.getAscentRate());
            ...
            if (Mvplan.DEBUG > 1) System.out.println("Now at next stop depth: "+nextStopDepth+" runtime: "+runTime);     
            ...
            if (setDecoGas(currentDepth) == true) { // If true we have changed gases
                if (inAscentCycle) {                // To switch gases during ascent need to force a waypoint
                    if(Mvplan.DEBUG > 1) ;
                    if (Mvplan.DEBUG > 1) System.out.println(" ... forcing waypoint for gas switch");
                    outputSegments.add(new SegmentAscDec(startDepth,currentDepth,Mvplan.prefs.getAscentRate(),tempGas,ppO2));  
                    startDepth=currentDepth;
                }
            }
            
            // Set next rounded stop depth
            nextStopDepth = (int)currentDepth - (int)Mvplan.prefs.getStopDepthIncrement(); 
        ...
        if(Mvplan.DEBUG > 1) model.printModel();        
        return SUCCESS;        
    } // ascend
    ...
    public void doGasCalcs()
    {
        ...
        while(it.hasNext()) {
            ...
            if(Mvplan.DEBUG > 1) System.out.println(" ... gas used: "+s.getGas()+" - "+(int)s.gasUsed()+" ("+(int)s.getGas().getVolume()+")");
        }     
    }
    ...
    */
    private boolean setDecoGas(double depth) 
    {
        ...
        if(Mvplan.DEBUG > 1) System.out.println("Evaluating deco gas at "+depth); 
        ...
        if (!Mvplan.prefs.getOcDeco())   return false;   // No OC deco so no bailout
        ...
        while(!finished & (currentGasIndex+1<gases.size())) {        // Is there another gas to switch to anyway ?
            ...
            if (g.getMod() >= depth) {              // Check MOD, Can move to this gas ?
                ...
                currentGas=g;  
                gasSwitch=true;
                if(Mvplan.DEBUG > 1) System.out.println(" ... changing gas to "+currentGas);                    
            } else
