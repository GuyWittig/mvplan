...
public class Gas implements Comparable, Serializable, Cloneable
{
    ...
    public static boolean validate(double fHe, double fO2, double mod) {
        ...
        if(fO2 == 0.0 && mod == 0.0)    // Leave empty gases alone to allow construction
            return passed;
        if(Prefs.getPrefs() != null) {   // Need to check that prefs exists. We can get to this point during the initilisation of the prefs object
            double d = ((mod+Prefs.getPrefs().getPConversion())/Prefs.getPrefs().getPConversion()*fO2);
            passed = (d <= Prefs.getPrefs().getMaxMOD()+0.05);  // Tolerance of 0.05 to prevent unneccessary failure due to rounding
        }
    ...
    public static double getMaxMod(double o) {
        return (Prefs.getPrefs().getMaxMOD()/o * Prefs.getPrefs().getPConversion())-Prefs.getPrefs().getPConversion();
    }
    /* 
     * Method to get a MOD based on O2 fraction and maximum ppO2
     */
    public static double getMod(double fO2, double ppO2) {
        return (ppO2/fO2 * Prefs.getPrefs().getPConversion())-Prefs.getPrefs().getPConversion();
    }
    /* 
     * Method to get a ppO2 based on O2 fraction and MOD
     */
    public static double getppO2(double f, double m) {
        return ( (m+Prefs.getPrefs().getPConversion())*f/Prefs.getPrefs().getPConversion());
    }
