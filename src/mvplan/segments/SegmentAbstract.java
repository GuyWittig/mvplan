...
public abstract class SegmentAbstract implements Serializable, Cloneable
{
    ...
    */
    public double getEnd() {        
        double pAbsolute = depth + Prefs.getPrefs().getPAmb();  // msw (fsw)
        ...
        if(setpoint > 0.0) {                     
            ...
            pInert = ((fHe+fN2)>0.0) ? pAbsolute - setpoint * Prefs.getPrefs().getPConversion() : 0.0;
            ppN2Inspired = pInert>0.0 ? (pInert * fN2)/(fHe+fN2) : 0.0;                                       
        } else 
            // Open circuit mode
            ppN2Inspired = (pAbsolute )*fN2;                                            
        double end = ppN2Inspired /0.79 - Prefs.getPrefs().getPAmb();
    ...
    {   
        ...
        return String.format("%1$3.0f"+Prefs.getPrefs().getDepthShortString()+" - %2$02d:%3$02dmin, %4$s @%5$3.1f",
